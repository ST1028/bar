...
const resetAllOrders = async (event) => {
  try {
    const user = getUserFromEvent(event);
    console.log('Admin reset-all requested by user:', user.email);
    
    // Get all patron and order data across all tenants (not just current user)
    // Scan for all items with pk starting with 'USER#' (user data, not menu data)
    const scanParams = {
    ...
    // Scan all pages to get complete dataset
    do {
      const currentScanParams = {
        ...scanParams,
        ...(lastEvaluatedKey && { ExclusiveStartKey: lastEvaluatedKey })
      };

      const result = await scanTable(TABLE_NAME, currentScanParams);
      allItems = allItems.concat(result.Items || []);
      lastEvaluatedKey = result.LastEvaluatedKey;
    } while (lastEvaluatedKey);

    const patronsAndOrders = allItems;
